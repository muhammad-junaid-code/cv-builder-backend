...
async def call_llm_atomic(client, key: str, model: str, url: str,
    ...
    if url == CEREBRAS_URL:
        ...
        per_call_timeout = 90
        # Groq hard caps output tokens at 4096 — clamp to avoid HTTP 400
    ...
    for attempt in range(2):
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                err_msg = r.text[:300]
            raise ValueError(f"HTTP {r.status_code} on {stage}: {err_msg}")
...
async def generate_cv_dynamic(req: CVRequest, client, key: str, model: str, 
    ...
    result = await call_llm_atomic(client, key, model, url, system_prompt, user_prompt,
                                    "FullCV", headers, max_tokens=4096, _deadline=_deadline)
